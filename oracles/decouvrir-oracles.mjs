#!/usr/bin/env node
/**
 * decouvrir-oracles.mjs — les oracles de forge-data, LUS SUR LE DISQUE, jamais recopiés d'une
 ...
 * mobilisée porte. Mesuré le 23/09/2026 : cette forge porte 15 fichiers `oracles/oracle-*.mjs`, et
 * AUCUN point d'entrée ne les énumère — chacun est appelé par son skill, et la recette
 * `oracles/self-test.mjs` ne lit le disque que pour ses registres machine (TF-1196). Aucune machine
 * ne pouvait donc dire ce qui AURAIT DÛ tourner sur un run qui mobilise forge-data.
 *
 ...
 * Ce script ne lance aucun oracle et n'écrit rien. Le juge du pilot
 ...
 * archives `Old`/`old`, `fixtures`, entrants `input`. Un oracle déposé demain est découvert sans
 * que personne ait à l'inscrire nulle part : c'est la propriété qui manquait.
 *
...
export const FORGE = "digit-ai-forge-data";
